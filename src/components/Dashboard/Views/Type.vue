      ...
      <div class="container-fluid ">
          <div class="row">
              <div class="col-lg-12 col-md-12">
                  <card>
                     <center><h4>Ici vous pouvez gérer les types</h4></center>
                  </card>
              </div>
          </div>
        <div class="row " >
          <div class="col-lg-7 col-md-7 ">
            <card>
              <h4 v-if="edit" slot="header" class="card-title">Modifier un type</h4>
              <h4 v-else slot="header" class="card-title">Enregistrer un type</h4>
              <label>Libelle*</label>
              <input type="text" name="LibelleType" placeholder="Libelle Type" v-validate="'required'" v-model="type.LibelleType" class="form-control" >
              <span style="color:red" v-show="errors.has('LibelleType')">{{ errors.first('LibelleType') }}</span>
              <br>
        ...
        <div class="row">
            <div class="  col-lg-12 col-md-12">
              <card>
                  <h4 slot="header" class="card-title">La liste des types</h4>
                <v-client-table 
                :data="types"
                :columns="columns" 
                :options="options"> 
     ...
     components: {
      ...
      data () {
        return {
          ...
          columns:['#','LibelleType','Modif','Supp'],  
              options: {
                  headings: {
                    'LibelleType': 'Libelle type',
                  },
                  sortable: ['LibelleType','#'],
                  filterable: ['LibelleType','#']
        }
        }
      },
      mounted(){
      ...
      editer(type,l)
        {
          ...
          this.edit=true
          this.id=type.id
          this.type.LibelleType=type.LibelleType       
        },
        update()
        {
          let l=this.l
          ...
          this.$validator.validate().then(result=>{
                if(result){
                  ...
                  .then(_=>{
                    ...
                    this.isload=false
                    this.types[l-1].LibelleType=this.type.LibelleType
