      ...
      <div class="container-fluid ">
          <div class="row">
              <div class="col-lg-12 col-md-12">
                  <card>
                     <center><h4>Ici vous pouvez gérer les fautes</h4></center>
                  </card>
              </div>
          </div>
        <div class="row " >
          <div class="col-lg-7 col-md-7 ">
            <card>
              <h4 v-if="edit" slot="header" class="card-title">Modifier un faute</h4>
              <h4 v-else slot="header" class="card-title">Enregistrer un faute</h4>
              <label>Libelle*</label>
              <input type="text" name="LibelleFaute" placeholder="Libelle faute" v-validate="'required'" v-model="faute.LibelleFaute" class="form-control" >
              <span style="color:red" v-show="errors.has('LibelleFaute')">{{ errors.first('LibelleFaute') }}</span>
              <br>
        ...
        <div class="row">
            <div class="  col-lg-12 col-md-12">
              <card>
                  <h4 slot="header" class="card-title">La liste des fautes</h4>
                <v-client-table 
                :data="fautes"
                :columns="columns" 
                :options="options"> 
     ...
     components: {
      ...
      data () {
        return {
          ...
          columns:['#','LibelleFaute','Modif','Supp'],  
              options: {
                  headings: {
                    'LibelleFaute': 'Libelle faute',
                  },
                  sortable: ['LibelleFaute','#'],
                  filterable: ['LibelleFaute','#']
        }
        }
      },
      mounted(){
        this.$http.get(this.state.url+this.url).then(response=>{
          this.fautes=response.data.fautes;
        })
      ...
      methods: {
        enreg()
        {
          ...
          .then(_=>{
              this.isload=true
             this.$http.post(this.state.url+this.url,this.faute).then(response=>{
             this.fautes.push(response.data.faute)
      ...
      editer(faute,l)
        {
          ...
          this.edit=true
          this.id=faute.id
          this.faute.LibelleFaute=faute.LibelleFaute        
        },
        update()
        {
          let l=this.l
          ...
          this.$validator.validate().then(result=>{
                if(result){
                  ...
                  .then(_=>{
                    this.isload=true
                    this.$http.put(this.state.url+this.url+'/'+this.id,this.faute).then(response=>{
                    this.isload=false
                    this.fautes[l-1].LibelleFaute=this.faute.LibelleFaute
